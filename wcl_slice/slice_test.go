...
package wcl_slice_test

import (
	"reflect"
	"testing"

	"github.com/wsva/common_lib/wcl_slice"
)

func TestCopyStrings(t *testing.T) {
	var src1 []string
	dest1 := wcl_slice.CopyStrings(src1)
	...
	src2 := []string{}
	dest2 := wcl_slice.CopyStrings(src2)
	...
	src3 := []string{"a", "c", "b"}
	dest3 := wcl_slice.CopyStrings(src3)
...
func TestSortStrings(t *testing.T) {
	src := []string{"a", "c", "b"}
	dest := wcl_slice.SortStrings(src)
...
func TestContainsString(t *testing.T) {
	src := []string{"aa", "bb", "cc"}
	if !wcl_slice.ContainsString(src, "bb", nil) {
	...
	if !wcl_slice.ContainsString(src, "ee", modifier) {
		t.Errorf("ContainsString didn't find the string by modifier")
	}

	src = make([]string, 0)
	if wcl_slice.ContainsString(src, "", nil) {
...
func TestRemoveString(t *testing.T) {
	...
	for _, tt := range tests {
		if got := wcl_slice.RemoveString(tt.input, tt.remove, tt.modifier); !reflect.DeepEqual(got, tt.want) {
